...
<script>
import { listSensitive, getSensitive, delSensitive, addSensitive, updateSensitive, exportSensitive } from "@/api/system/sensitive";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除敏感词管理编号为"' + ids + '"的数据项?', "警告", {
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有敏感词管理数据项?', "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(() => {
        this.exportLoading = true;
        return exportSensitive(queryParams);
      }).then(response => {
