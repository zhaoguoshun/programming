...
public class SysLoginController
{
    ...
    public AjaxResult getInfo()
    {
        ...
        SysUser loginUserUser = loginUser.getUser();

        SysUser user = userService.selectUserById(loginUserUser.getUserId());

        // 角色集合
        Set<String> roles = permissionService.getRolePermission(loginUserUser);
        // 权限集合
        Set<String> permissions = permissionService.getMenuPermission(loginUserUser);
        AjaxResult ajax = AjaxResult.success();
        //获取部门(公司名称)
        SysDept dept = sysDeptService.selectDeptById(loginUserUser.getDeptId());
        ...
        int count = detailService.count(loginUserUser.getUserId());
        int focusUserCount = focusService.focus(loginUserUser.getUserId());
        int quiltUserCount = focusService.focusQuiltUserId(loginUserUser.getUserId());
